...
function update_grub_file() {
  ...
  echo "GRUB_THEME=/boot/${GRUB_NAME}/themes/Sheffield/theme.txt" >> /etc/default/grub
}

function copy_sheffield_files() {
  echo -e "\e[1m\e[32m==> \e[97mDownloading files...\e[0m"
  git clone https://github.com/osmanonurkoc/Sheffield-Grub-Theme /tmp/Sheffield-Grub-Theme
  echo -e "\e[1m\e[32m==> \e[97mCopying files...\e[0m"
  cp -rf /tmp/Sheffield-Grub-Theme/Sheffield /boot/${GRUB_NAME}/themes/
}
